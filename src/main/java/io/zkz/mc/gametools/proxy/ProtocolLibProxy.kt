...
object ProtocolLibProxy : InjectionComponent {
    ...
    fun setupGlowing(plugin: Plugin?) {
        ...
        protocolManager.addPacketListener(
            object : PacketAdapter(plugin, PacketType.Play.Server.ENTITY_METADATA) {
                override fun onPacketSending(event: PacketEvent) {
                    ...
                    if (aboutPlayerTeam == recieverTeam) {
                        val watchableObjectList: List<WrappedWatchableObject> = packet.watchableCollectionModifier.read(0)
